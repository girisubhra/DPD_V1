...
Input_IComp=readmatrix('D:\PhD_DPD_Project\DPD_V1\Input_I.xlsx');
Input_QComp=readmatrix('D:\PhD_DPD_Project\DPD_V1\Input_Q.xlsx');
...
for ii=1:length(Xact)
    trainsamp(ii)=900+ii;%start from 500 samples
end
Ka=10;
La=3;
...
Coeff_ls=readmatrix('D:\PhD_DPD_Project\DPD_V1\DPD_Co-efficient.txt');
%{
Ka=5;
La=3;
...
a = real(Ypredict);
writematrix(a,'D:\PhD_DPD_Project\DPD_V1\7_Ydpd_I_Sample.xlsx')
b = imag(Ypredict);
writematrix(b,'D:\PhD_DPD_Project\DPD_V1\8_Ydpd_Q_Sample.xlsx')
%display(b);
c = real(Yact_test);
